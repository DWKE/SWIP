...
% H(s)
figure(1)
H = tf([Kp_ACC Kd_ACC],[1 Kd_ACC Kp_ACC]);
bode(H); grid on
...
% Run Simulation
out = sim('ACC_Space_Multi_Simul.slx');
...
hold on
plot(out.ControlMode1(:,1),out.ControlMode1(:,2)*50+100)
plot(out.ControlMode2(:,1),out.ControlMode2(:,2)*50)
hold off
grid on;
ylim([0 max([delta_h1_0 delta_12_0])+100])
xlabel('time(sec)'); ylabel('Distance(m)')
legend('L_{Head,Car1}','L_{Car1,Car2}', ...
