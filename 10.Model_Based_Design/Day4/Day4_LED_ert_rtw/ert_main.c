...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Day4_LED_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(Day4_LED_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Day4_LED_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  mwRaspiInit();
  MW_launchPyserver();
  rtmSetErrorStatus(Day4_LED_M, 0);
  ...
  errorCode = extmodeInit(Day4_LED_M->extModeInfo, &rtmGetTFinal(Day4_LED_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Day4_LED_M, true);
    }
  }
